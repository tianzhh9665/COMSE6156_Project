...
public class PlayerServiceImpl implements PlayerService {
    ...
    public ResponseDTO info(String playerUuid, String token) {
        ...
        PlayerInventoryDO invDO = new PlayerInventoryDO();
        invDO.setPlayerUuid(playerUuid);
        
        List<PlayerInventoryDO> invInfoList = playerInventoryMapper.selectList(new QueryWrapper<>(invDO));
        returnDTO.setInventoryInfo(invInfoList);
    ...
    public ResponseDTO deletePlayer(PlayerDeleteDTO dto){
        ...
        PlayerInventoryDO invDO = new PlayerInventoryDO();
        invDO.setPlayerUuid(dto.getPlayerId());
        
        if(playerInventoryMapper.delete(new QueryWrapper<>(invDO)) < 0) {
        	return new ResponseDTO(500, "ERROR: delete inventory info failed", null);
        }
    ...
    public ResponseDTO trade(PlayerTradeRequestDTO dto) {
        ...
        if(dto.getType() == PlayerTradeTypeEnum.MONEY_TO_MONEY.getType()) {
        	...
        	if(player1.getStatus().equals(PlayerStatusEnum.FROZEN.getStatus())) {
        		return new ResponseDTO(502, "ERROR: player 1 is frozen",null);
        	}
        	...
        	if(player2.getStatus().equals(PlayerStatusEnum.FROZEN.getStatus())) {
        		return new ResponseDTO(502, "ERROR: player 2 is frozen",null);
        	}
